...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.column_names = self.controller.db.get_column_names()
    ...
    def create_custom_filter_ui(self, layout):
        ...
        # Add mrl_matched and fulfillment_matched filter options
        process_layout = QHBoxLayout()
        process_layout.addWidget(QLabel("MRL Matched:"))
        self.mrl_matched_group = QButtonGroup(self)
        for option in ["All", "Yes", "No"]:
            radio = QRadioButton(option)
            self.mrl_matched_group.addButton(radio)
            process_layout.addWidget(radio)
        process_layout.addSpacing(20)
        process_layout.addWidget(QLabel("Fulfillment Matched:"))
        self.fulfillment_matched_group = QButtonGroup(self)
        for option in ["All", "Yes", "No"]:
            radio = QRadioButton(option)
            self.fulfillment_matched_group.addButton(radio)
            process_layout.addWidget(radio)
    ...
    def populate_report_names(self):
        ...
        names = self.controller.get_report_names()
    ...
    def update_sheet_names(self, selected_name):
        ...
        if not selected_name:
            ...
            return
        # Corrected line: Use self.controller to get sheet names
        sheet_names = self.controller.get_sheet_names(selected_name)
    ...
    def load_report(self):
        ...
        data = self.controller.load_report_data(selected_name, selected_sheet)
    ...
    def find_matches(self):
        ...
        try:
            potential_matches = self.controller.find_matches(selected_records)
            if not potential_matches:
                self.show_error("No potential matches found.")
            else:
                report_info = {
                    'report_name': self.report_name_combo.currentText(),
                    'report_date': selected_records[0].get('report_date', ''),
    ...
    def open_matching_detail_window(self, potential_matches, report_info):
        if not hasattr(self, 'matching_detail_window') or not self.matching_detail_window.isVisible():
            self.matching_detail_window = MatchingDetailWindow(potential_matches, self.controller.db, report_info)
    ...
    def apply_custom_filters(self):
        ...
        # Add MRL Matched filter
        mrl_matched = next((btn for btn in self.mrl_matched_group.buttons() if btn.isChecked()), None)
        if mrl_matched and mrl_matched.text() != "All":
            value = mrl_matched.text()
            if value == 'Yes':
                # For 'Yes', match records where mrl_matched is 1 or True
                filters.append(('mrl_matched', 'equals', 1))
            elif value == 'No':
                # For 'No', match records where mrl_matched is 0 or NULL
                filters.append(('mrl_matched', 'equals_or_null', 0))

        # Add Fulfillment Matched filter
        fulfillment_matched = next((btn for btn in self.fulfillment_matched_group.buttons() if btn.isChecked()), None)
        if fulfillment_matched and fulfillment_matched.text() != "All":
            value = fulfillment_matched.text()
            if value == 'Yes':
                filters.append(('fulfillment_matched', 'equals', 1))
            elif value == 'No':
                filters.append(('fulfillment_matched', 'equals_or_null', 0))
        ...
        # Apply filters via the controller
        filtered_data = self.controller.apply_filters(filters, selected_name, selected_sheet)
    ...
    def apply_preset_filter(self):
        ...
        # Apply preset filter via the controller
        filtered_data = self.controller.apply_preset_filter(selected_preset)
        self.display_results(filtered_data)
    ...
    def validate_sql(self):
        ...
        # Validate the SQL query via the controller
        is_valid, message = self.controller.validate_sql(sql_query)
        if is_valid:
    ...
    def apply_sql_query(self):
        ...
        # Execute the SQL query via the controller
        result = self.controller.execute_sql(sql_query)
